...
public class BuffAgilitySet implements IBuffHandler {
	private int myLevel = 0;
	public BuffAgilitySet(int armorLevel)
	{
		myLevel = armorLevel;
	}
    @Override
    public void applyPlayerBuffs(EntityPlayer player, int itemCount) {
    	switch(myLevel)
    	{
    		case 0:
    		{
    			if (itemCount == 4)
    			{
    				player.addPotionEffect(new PotionEffect(ScourgeCraftCore.configPotion.stepUp.id, 40));
    			}
    			break;
    		}
    		case 1:
